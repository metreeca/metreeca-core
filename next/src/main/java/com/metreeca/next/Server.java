	...
	@Override public void handle(final Request request, final Response response) {
		...
		try (final RepositoryConnection ignored=graph.connect()) { // process the request on a single connection
			...
			handler // wrap on the fly to make the original handler available to overridden Handler.wrap()
					...
					.wrap(rewriter())
					.wrap(postprocessor()) // outside rewriter: sees original URLs
			...
			if ( !response.committed() ) {
				response.status(Response.InternalServerError).cause(e).json(object(
						field("notes", "unable to process request: see server logs for details")
				));
			}
		...
		return handler -> (request, response) -> handler.exec(
				...
				writer -> {
					...
					writer.copy(request)
							...
							.parameters(request.parameters().map(h -> new SimpleImmutableEntry<>(
									h.getKey(), h.getValue().stream().map(rewriter::internal).collect(toList())
							)))

							.headers(request.headers().map(h -> new SimpleImmutableEntry<>(
									h.getKey(), h.getValue().stream().map(rewriter::internal).collect(toList())
							)))

							.body(
									() -> rewriter.internal(request.input()),
									() -> rewriter.internal(request.reader())
							);
				},
				...
				reader -> {
					...
					response.copy(reader)
							...
							.headers(reader.headers().map(h -> new SimpleImmutableEntry<>(
									h.getKey(), h.getValue().stream().map(rewriter::external).collect(toList())
							)));
