...
@FunctionalInterface public interface Handler {
	...
	public static final Handler Empty=new Handler() {
		...
		@Override public Handler chain(final Handler handler) {
			return handler;
		}
	...
	public static Handler sysadm(final Handler handler) { // !!! remove after testing shape-based authorization
	...
	public static void unsupported(final Tool.Loader tools,
			final Request request, final Response response, final BiConsumer<Request, Response> sink) {
		...
		sink.accept(request, response.setStatus(Response.MethodNotAllowed)
				.setText("unsupported "+request.getMethod()+" request ["+request.getTarget()+"]"));
	}

	public static void unauthorized(final Tool.Loader tools,
			final Request request, final Response response, final BiConsumer<Request, Response> sink) {
		...
		if ( false ) {
			...
			sink.accept(request, response.setStatus(Response.NotFound));

		} else if ( request.getUser().equals(RDF.NIL) ) {

			// no WWW-Authenticate header: authentication is managed by the server

			sink.accept(request, response.setStatus(Response.Unauthorized));

		} else {

			sink.accept(request, response.setStatus(Response.Forbidden));

		}

	}

	public static void unimplemented(final Tool.Loader tools,
			final Request request, final Response response, final BiConsumer<Request, Response> sink) {

		sink.accept(request, response.setStatus(Response.NotImplemented));

	}


	public void handle(final Tool.Loader tools,
			final Request request, final Response response, final BiConsumer<Request, Response> sink);


	public default Handler chain(final Handler handler) {
