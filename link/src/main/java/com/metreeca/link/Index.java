...
public final class Index {
	...
	public static final Tool<Index> Tool=tools -> {
		...
		tools.get(Server.Tool).hook(index::load);
	...
	public Index(final Loader tools) {
		this.tools=tools;
		this.server=tools.get(Server.Tool);
		this.graph=tools.get(Graph.Tool);
	}


	public Handler get(final String path) { // !!! optimize?
	...
	public Index insert(final Cell specs) {
		...
		return exec(index -> {
			...
			if ( collection || container ) {
				...
				create.ifPresent(hook -> hooks.put(Request.POST, hook));
				mutate.ifPresent(hook -> hooks.put(Request.ANY, hook));
			...
			if ( collection ) { // ancillary resource port
				...
				final Handler handler=new Resource(tools, detail);

				final Map<String, String> hooks=new HashMap<>();

				update.ifPresent(hook -> hooks.put(Request.PUT, hook));
				delete.ifPresent(hook -> hooks.put(Request.DELETE, hook));
				mutate.ifPresent(hook -> hooks.put(Request.ANY, hook));
	...
	public Index insert(final String path, final Handler handler, final Map<IRI, Value> properties) {
