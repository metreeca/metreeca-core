...
package com.metreeca.gcloud.formats;

import com.metreeca.gcloud.GCloud;
import com.metreeca.gcloud.services.Datastore;
...
final class EntityDecoder {
	...
	private EntityValue entity(final JsonObject object, final Shape shape) {
		...
		final String id=Optional.ofNullable(object.get(GCloud.id))
				.filter(value -> value instanceof JsonString)
				.map(value -> ((JsonString)value).getString())
				.orElse("");

		final String type=Optional.ofNullable(object.get(GCloud.type))
				.filter(value -> value instanceof JsonString)
				.map(value -> ((JsonString)value).getString())
		...
		final FullEntity.Builder<?> builder=id.isEmpty()
				...
				: FullEntity.newBuilder(datastore.key(type.isEmpty() ? GCloud.Resource : type, id));

		final Map<Object, Shape> fields=fields(shape);

		object.forEach((name, json) -> {
			if ( !name.equals(GCloud.id) && !name.equals(GCloud.type) ) {
