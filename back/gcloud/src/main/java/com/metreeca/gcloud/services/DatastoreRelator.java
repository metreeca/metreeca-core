...
import static com.metreeca.gcloud.formats.EntityFormat.entity;
import static com.metreeca.gcloud.services.Datastore.datastore;
import static com.metreeca.gcloud.services.Datastore.value;
...
final class DatastoreRelator extends DatastoreProcessor {
	...
	private static final Shape TermShape=and(
			field(GCloud.label, and(optional(), datatype(STRING)))
	);

	private static final Shape TermsShape=and(
			field(GCloud.terms, and(multiple(),
					field(GCloud.value, and(required(), TermShape)),
					field(GCloud.count, and(required(), datatype(LONG)))
			))
	);
	...
	private static final Shape StatsShape=and(
			...
			field(GCloud.count, and(required(), datatype(LONG))),
			field(GCloud.min, and(optional(), TermShape)),
			field(GCloud.max, and(optional(), TermShape)),

			field(GCloud.stats, and(multiple(),
					field(GCloud.type, and(required(), datatype(STRING))),
					field(GCloud.count, and(required(), datatype(LONG))),
					field(GCloud.min, and(required(), TermShape)),
					field(GCloud.max, and(required(), TermShape))
			))
	...
	private static String label(final Value<?> value) {
		return Optional.of(value)
				...
				.filter(e -> e.contains(GCloud.label))
				.map(e -> e.getString(GCloud.label))
				.orElse("");
	}
	...
	private Function<Response, Response> items(final String path, final Items items) {
		...
		final Entity container=Entity.newBuilder(datastore.key(GCloud.Resource, path))

				.set(GCloud.contains, this.<List<EntityValue>>entities(path, shape, orders, offset, limit, entities -> entities
		...
		return response -> response
				...
				.shape(field(GCloud.contains, convey(shape)))
	...
	private Function<Response, Response> terms(final String path, final Terms terms) {
		...
		final Entity container=Entity.newBuilder(datastore.key(GCloud.Resource, path))
				...
				.set(GCloud.terms, this.<List<EntityValue>>values(path, terms.getShape(), terms.getPath(), values -> values
						...
						.map(entry -> FullEntity.newBuilder()
								...
								.set(GCloud.value, entry.getKey())
								.set(GCloud.count, entry.getValue())
	...
	private Function<Response, Response> stats(final String path, final Stats stats) {
		...
		final class Range {
			...
			private void set(final BaseEntity.Builder<?, ?> container) {
				...
				container.set(GCloud.count, count);
				container.set(GCloud.min, min);
				container.set(GCloud.max, max);
		...
		final Entity.Builder container=Entity.newBuilder(datastore.key(GCloud.Resource, path));

		if ( ranges.isEmpty() ) {

			container.set(GCloud.count, 0L);
			...
			container.set(GCloud.stats, ranges.entrySet().stream() // type-specific stats
					...
					.map(entry -> {
						...
						item.set(GCloud.type, entry.getKey());
	...
	private EntityQuery query(final String path, final Shape shape,
		...
		final Filter ancestor=hasAncestor(datastore.key(GCloud.Resource, path));
