...
public final class DatastoreInferencer extends Inspector<Shape> {
	...
	@Override public Shape probe(final Meta meta) {
		return meta.getLabel().equals(Shape.Hint) ? and(meta, datatype(GAE.Entity)) : meta;
	}


	@Override public Shape probe(final Datatype datatype) {
		return datatype.getName().equals(GAE.Boolean) ? and(datatype, in(false, true), maxCount(1)) : datatype;
	}

	@Override public Shape probe(final Clazz clazz) {
		return and(clazz, datatype(GAE.Entity));
	}
	...
	@Override public Shape probe(final In in) {
		...
		final Set<String> types=values.stream().map(GAE::type).collect(toSet());
	...
	@Override public Shape probe(final Field field) {
		...
		return and(field(name, shape), datatype(GAE.Entity));
	}
