...
import static com.metreeca.back.sparql.GraphTest.export;
...
final class GraphsTest {
	...
	private Request put(final Request request) {
		return request.method(Request.PUT).body(InputBody.input(), () ->
	...
	private Request post(final Request request) {
		return request.method(Request.POST).body(InputBody.input(), () ->
				new ByteArrayInputStream(encode(Rest).getBytes(Codecs.UTF8))
		);
	}
