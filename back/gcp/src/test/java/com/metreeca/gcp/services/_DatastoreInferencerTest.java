...
final class _DatastoreInferencerTest {
	...
	@Test void testHint() {
		...
		assertImplies("hinted shapes are resources",
				hint("/entities/"), datatype(ValueType.ENTITY));
	...
	@Test void testType() { // !!! improve testing of multiple implications
		...
		assertImplies("xsd:boolean has closed range",
				datatype(ValueType.BOOLEAN), and(maxCount(1), in(false, true)));
	...
	@Test void testClazz() {
		assertImplies("classed values are resources",
				clazz("Entity"), datatype(ValueType.ENTITY));
	}
	...
	@Test void testRange() {
		...
		assertImplies("if unique, focus values share the datatype of the allowed value set",
				in(1L, 2L), and(maxCount(2), datatype(ValueType.LONG)));
	...
	@Test void testField() {
		...
		assertImplies("field subjects are resources",
				field("field", and()), datatype(ValueType.ENTITY));

		assertImplies("nested shapes are expanded",
				field("field", clazz("Class")), datatype(ValueType.ENTITY),
				(s, i) -> and(field(s.getName(), and(and(s.getShape(), i), datatype(ValueType.ENTITY))), datatype(ValueType.ENTITY)));
	}

	@Test void testConjunction() {
		assertImplies("nested shapes are expanded", and(clazz("Class")), datatype(ValueType.ENTITY),
	...
	@Test void testDisjunction() {
		assertImplies("nested shapes are expanded", or(clazz("Class")), datatype(ValueType.ENTITY),
