...
public final class Server {
	...
	private static final Logger logger=Logger.getLogger(Server.class.getName()); // !!! migrate to Trace
	...
	public static final Tool<Server> Tool=Server::new;
	...
	public Server(final Tool.Loader tools) {
		this(tools, new Router());
	}

	public Server(final Tool.Loader tools, final Handler... handlers) {
		this(tools, asList(handlers));
	}

	public Server(final Tool.Loader tools, final Iterable<Handler> handlers) {
		...
		final Gate gate=tools.get(Gate.Tool);

		this.handler=StreamSupport

				.stream(handlers.spliterator(), false)
				.reduce(gate::authorize, Handler::chain)

				.chain(new Dispatcher(map( // default method post-processors

						entry(Request.DELETE, this::delete),
						entry(Request.ANY, Handler.Empty)
	 ...
	 * <ol>
	 *
	 * <li>the absolute base URL of this server as {@linkplain Request#getBase() derived} from the request object;</li>
	 *
	 ...
	 * @return this server
	 */
	public Server hook(final BiConsumer<String, String> hook) {
	...
	public Server serve(final Tool.Loader tools,
		...
		} catch ( final LinkException e ) { // !!! remove

			defaults(tools, request, response.setStatus(e.getStatus())
					.setText(e.getReport())
					.setCause(e.getCause()), sink);

		} catch ( final RuntimeException e ) {

			defaults(tools, request, response.setStatus(Response.InternalServerError)
					.setText("unable to process request: see server logs for details")
					.setCause(e), sink);
		...
		if ( response.getStatus()/100 != 2 ) { sink.accept(request, response); } else {
			...
			if ( cell.isEmpty() ) { sink.accept(request, response); } else { // report leftovers
				...
				sink.accept(request, new Response().setStatus(Response.InternalServerError)
						.setText("resource still referenced after deletion")
						.setCause(new IllegalStateException("leftover references after "+format(target)+" deletion:\n"+references)));
	...
	private void defaults(final Tool.Loader tools,
			final Request request, final Response response, final BiConsumer<Request, Response> sink) {
		...
		logger.log(status < 400 ? Level.INFO : status < 500 ? Level.WARNING : Level.SEVERE,
				String.format("%s %s > %d", request.getMethod(), request.getTarget(), status),
				response.getCause());
