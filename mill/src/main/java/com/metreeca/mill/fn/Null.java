...
public class Null implements Function {
	...
	@Override public Value evaluate(final ValueFactory factory, final Value... args) {
		...
		if ( args.length >= 1 ) {
			...
			final Value target=args[0];

			for (int i=1; i < args.length; i++) {
				if ( target.equals(args[i]) ) {
					throw new ValueExprEvaluationException("target value matches null value");
				}
			}
			...
			throw new IllegalArgumentException(String.format("usage %s(<target value>, <null value>+)", Name));
