 ...
 * session token in a HTTP only session cookie, the session expiry date in milliseconds in the custom {@value
 * SessionHeader} header and a JSON payload containing the user profile included in the {@linkplain Permit#profile()
 ...
 * custom {@value SessionHeader} header and an empty JSON payload; malformed and unknown session tokens are
 * ignored.</p>
 *
 * <pre>{@code 200 OK
 ...
 * …}</pre>
 *
 * <p><em>Successful token validation</em> / The response includes the response generated by the secured handler;
 * session cookie and expiry date in milliseconds may be included if the session was automatically extended.</p>
 *
 * <pre>{@code <###> <Status>
 * Set-Cooke: ID=<token>; Path=<base>; HttpOnly; Secure; SameSite=Lax // optional
 * X-ID: 1234567890 // optional
 *
 * … }</pre>
 *
...
public final class Manager implements Wrapper {
	...
	public static final String SessionHeader="X-ID";
						...
						.process(ticket -> {
							...
							if ( ticket.isEmpty() ) {
								...
								return Value(response.status(Response.OK)
										...
										.header(SessionHeader, String.valueOf(0))
								...
								return login(ticket).value(permit -> {
									...
									final long issued=clock.time();
									final long expiry=issued+soft;

									return response.status(Response.Created)

											.header("Set-Cookie", cookie(request, permit.id(), issued, expiry))
											.header(SessionHeader, String.valueOf(expiry))
						...
						.map(claims -> lookup(claims.getSubject()).fold( // valid token >> look for permit
								...
								permit -> handler.handle(request // permit found > authenticate and process request
									...
									if ( expiry-now < soft*90/100 ) { // extend if residual lease is < 90% soft timeout
										response
												.header("Set-Cookie", cookie(request, permit.id(), issued, extend))
												.header(SessionHeader, String.valueOf(extend));
									}
