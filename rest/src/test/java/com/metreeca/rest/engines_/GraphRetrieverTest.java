...
final class GraphRetrieverTest extends GraphProcessorTest {
	...
	private Collection<Statement> query(final Query query) {
		return tool(Graph.Factory).query(connection -> {
			return query.map(new GraphRetriever(connection, Form.root, true));
		});
	}


	private Collection<Statement> virtuoso(final Collection<Statement> model) { // ;(virtuoso) counts reported as xsd:int
		return model.stream()
