...
final class GraphRetriever extends GraphProcessor implements Query.Probe<Collection<Statement>> {
	...
	private static final Function<Shape, _Profile> profile=memoizable(
			_Profile::profile
	);
	...
	private static boolean container(final Value value) {
		return value instanceof IRI && value.stringValue().endsWith("/");
	}
	...
	@Override public Collection<Statement> probe(final Edges edges) {
		...
		final Shape selector=selector(shape);
	...
	private Shape selector(final Shape shape) {
		return and(container(resource) ? shape.map(profile).anchor(resource) : all(resource), shape.map(filter));
	}
