...
abstract class _SPARQL { // ! refactor
	...
	public String compile() { return new _SPARQLBuilder().text(code()).text(); }
	...
	private final class FilterProbe implements Shape.Probe<Object> {
		...
		@Override public Object probe(final Any any) {
			...
			return list("\fvalues ", term(source), " {\n",
					items(any.getValues().stream().map(_SPARQL.this::term).collect(toList()), "\n"),
					"\n}\f");
		}
