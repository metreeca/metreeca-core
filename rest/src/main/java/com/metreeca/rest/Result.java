...
public abstract class Result<V, E> {
	...
	public <R> Result<R, E> value(final Function<V, R> mapper) {
		...
		return fold(v -> Value(requireNonNull(mapper.apply(v), "null mapper return value")), Result::Error);
	}
	...
	public <R> Result<V, R> error(final Function<E, R> mapper) {
		...
		return fold(Result::Value, e -> Error(requireNonNull(mapper.apply(e), "null mapper return value")));
	}
